...
module.exports = class {
	...
	async showDatatypeList(limit, reverse, lower_bound, upper_bound) {
		this.eventCallback('showDatatypeList()')
		const osb = this
		return new Promise(async function(resolve, reject) {
			const datatypeList = await osb.eosjs.getTableRows({
		        json: true,
	...
	async showDataList(limit, reverse, lower_bound, upper_bound) {
		this.eventCallback('showDataList()')
		const osb = this
		return new Promise(async function(resolve, reject) {
	...
	async showDataListByDatatype(limit, reverse, datatype_name) {
		this.eventCallback('showDataListByDatatype()')
		const osb = this
		return new Promise(async function(resolve, reject) {
			const datatypeList = await osb.eosjs.getTableRows({
		        json: true,
			...
			datatypeList.rows.forEach(async function(datatype){
				if (datatype.datatype_name === datatype_name) {
					const dataList = await osb.eosjs.getTableRows({
				        json: true,
				        code: osb.osb.dataTradeContractName,
				        scope: osb.osb.dataTradeContractName,
				        table: "data",
				        table_key: "bydatatype",
				        limit: limit,
				        reverse: reverse,
				        lower_bound: datatype.datatype_id,
				        upper_bound: datatype.datatype_id,
				        key_type: "i64",
				        index_position: 2
				    }).catch(function(error) {
				        osb.eventCallback(error)
	...
	async showBuyhistoryListByBuyer(limit, reverse, buyer_name) {
		this.eventCallback('showBuyhistoryListByBuyer()')
		const osb = this
		return new Promise(async function(resolve, reject) {
			const buyhistoryList = await osb.eosjs.getTableRows({
		        ...
		        limit: limit,
		        reverse: reverse,
		        lower_bound: buyer_name,
		        upper_bound: buyer_name,
		        key_type: "i64",
		        index_position: 2
		    }).catch(function(error) {
		        osb.eventCallback(error)
		        reject('failed to retrieve buyhistory list')
		    });
			resolve(buyhistoryList)
		})
	}

	async addDatatype(name, detailFieldNames) {
		const result = await osb.eosjs.transaction({
			actions: [{
		      ...
		      data: {
		    	user: osb.osb.account,						//데이터 제공자 계정명
	    	    datatype_name: name,						//데이터 타입명
	    	    detail_fields_num: detailFieldNames.length,	//데이터 구매 가격 (단위: 0.0001 OSB)
	    	    detail_fields: detailFieldNames,			//데이터 추가 필드 값
		      },
		    }]
		}, {
		  blocksBehind: 3,
...
//		console.log('adddatabegin()')
	...
	async registerData(data, datatypename, detailFields, price, period, decryptKeyList) {
		const osb = this
		return new Promise(async function(resolve, reject) {
			const checktime_1 = new Date().getTime()
			console.log("Check time: Started at " + checktime_1)
			...
			const checktime_2 = new Date().getTime()
			const elapsed_1 = checktime_2 - checktime_1
			console.log("Elapsed time: Preparing data and Executing transaction adddatabegin - " + elapsed_1)
			...
			const checktime_3 = new Date().getTime()
			const elapsed_2 = checktime_3 - checktime_2
			console.log("Elapsed time: Uploading data fragments - " + elapsed_2)
			...
			const checktime_4 = new Date().getTime()
			const elapsed_3 = checktime_4 - checktime_3
			console.log("Elapsed time: Executing transaction adddataend - " + elapsed_3)
	...
	requestData(dataId, buyId, buyerPrivateKey) {
		this.eventCallback(`requestData(${dataId}, ${buyerPrivateKey})`)
	...
	getDataHash(data) {
		...
		const checkElapsedTimeStart = new Date().getTime()
		...
		const elapsedTime = new Date().getTime() - checkElapsedTimeStart
		this.eventCallback("Elapsed time: getDataHash - " + elapsedTime)
	...
	async uploadDataToIdfs(idfs, data) {
		...
		return new Promise(async function(resolve, reject) {
    		...
    		if (response.data.encrypt_key === encryptKey) {
    			const elapsedTime = new Date().getTime() - checkElapsedTimeStart
    			osb.eventCallback("Elapsed time: Uploading data fragment - " + elapsedTime)
	...
	async encode(data, encryptKey) {
	    ...
	    const elapsedTime = new Date().getTime() - checkElapsedTimeStart
		this.eventCallback("Elapsed time: encode - " + elapsedTime)
	    return buf
	}
	...
	async decode (buf, decryptKey) {
	    ...
	    const elapsedTime = new Date().getTime() - checkElapsedTimeStart
		this.eventCallback("Elapsed time: decode - " + elapsedTime)
		return data
	}
	...
	fragmentize(data, numberOfFragment) {
		...
		var frag = []
		for (var i=0; i < numberOfFragment; i++) {
			const sourceStart = parseInt(data.length / numberOfFragment * i)
			const sourceEnd = parseInt(data.length / numberOfFragment * (i + 1))
			frag[i] = Buffer.alloc( Math.ceil( sourceEnd - sourceStart ) )
			data.copy(frag[i], 0, sourceStart, sourceEnd)
		}
		
		const elapsedTime = new Date().getTime() - checkElapsedTimeStart
		this.eventCallback("Elapsed time: Fragmentization - " + elapsedTime)
		return frag
	}
	...
	merge(fragments, mergedDataLength) {
		...
		for (var i=0; i < fragments.length; i++) {
			fragments[i].copy(mergedData, index, 0, fragments[i].length)
			index += fragments[i].length
		}

		if (index != mergedDataLength) {
			this.eventCallback("Error - the merged data length is unmatched")
		}

	    const elapsedTime = new Date().getTime() - checkElapsedTimeStart
		this.eventCallback("Elapsed time: Merging fragments - " + elapsedTime)
